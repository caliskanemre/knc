...
export default function NextHeader({ locale = 'tr', defaultLocale = 'tr', asPath = '/' }) {
  ...
  // Kategoriler i18n etiketleriyle - public klasöründeki resimlerle
  const categories = [
    { key: 'hennaset', label: t('HennaSet', 'Kına Seti'), icon: '/hennaset.jpg' },
    { key: 'handkerchief', label: t('HalayHandkerchief', 'Mendil'), icon: '/mendil.jpg' },
    { key: 'tambourine', label: t('Tambourine', 'Tef'), icon: '/tamborine.jpg' },
    { key: 'veil', label: t('Veil', 'Tül'), icon: '/veil.jpg' },
    { key: 'gift', label: t('Gift', 'Hediye'), icon: '/gift.jpg' },
    { key: 'ornament', label: t('Ornament', 'Süs'), icon: '/ornament.jpg' },
  ...
  return (
    <>
      ...
      <AppBar position="fixed" color="transparent" elevation={0} sx={{ borderBottom: '1px solid #eee', backdropFilter: 'saturate(180%) blur(8px)', bgcolor: 'rgba(255,255,255,0.9)' }}>
        <Toolbar sx={{ minHeight: { xs: 56, sm: 64 } }}>
          ...
          >
            ...
            {/* Siparişler ve Çık��ş: sadece giriş yapmış kullanıcılar için */}
            {isLoggedIn ? (
              <>
      ...
      {/* Login Dialog */}
      <Dialog open={openLoginDialog} onClose={() => setOpenLoginDialog(false)} PaperProps={{ sx: { borderRadius: 2 } }}>
        <DialogTitle>{t('Login', 'Giriş Yap')}</DialogTitle>
        <DialogContent>
          <Box component="form" noValidate sx={{ mt: 1 }}>
            <TextField
              ...
              fullWidth
              id="email"
              label={t('Email Address', 'E-posta Adresi')}
              name="email"
              autoComplete="email"
              autoFocus
              value={loginEmail}
              onChange={(e) => setLoginEmail(e.target.value)}
              sx={{ '& .MuiOutlinedInput-root': { borderColor: '#8B0000' } }}
            />
            <TextField
              margin="normal"
              required
        ...
        <DialogActions>
          ...
          <Button
            onClick={async () => {
              try {
                const res = await axios.post(`${baseURL}/auth/login`, { email: loginEmail, password: loginPassword });
                ...
                let count = 0;
                if (user.username) {
                  const cartRes = await axios.get(`${baseURL}/cart/${encodeURIComponent(user.username)}`, {
                    headers: { Authorization: `Bearer ${token}` }
                  });
                ...
                alert(t('Login failed. Please check your credentials and try again.', 'Giriş başarısız oldu. Lütfen kimlik bilgilerinizi kontrol edin ve tekrar deneyin.'));
              }
            }}
            color="primary"
          >
