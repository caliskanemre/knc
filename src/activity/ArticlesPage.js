...
export default function ArticlesPage() {
    ...
    // Get unique topics from the data for filtering
    const topics = Array.from(new Set(articlesData.map((a) => a.topic)));

    // Filter articles if a topic is selected
    const filteredArticles = topicFilter === 'All'
        ? articlesData
        : articlesData.filter((article) => article.topic === topicFilter);
                    ...
                    {filteredArticles.map((article) => (
                        <Grid key={article.id} item xs={12} sm={6} md={4}>
                            ...
                            >
                                ...
                                {/* "Read More" - or link to single article page */}
                                <Box sx={{p: 2, pt: 0}}>
                                    <Button
                                        variant="outlined"
                                        size="small"
                                        component={Link}
                                        to={`/articles/${article.id}`} // Create a detail route for each article
                                        sx={{
                                            borderColor: '#8B0000',
                                            color: '#8B0000',
                                            ':hover': {
                                                borderColor: '#8B0000',
                                                backgroundColor: '#f9ecec' // Subtle color on hover
                                            }
                                        }}
                                    >
                                        Read More
